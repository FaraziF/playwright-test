...
const getTestResult = (suiteName, filePath, coverage) => {
  ...
  if (!testResult) {
    console.log(
      `Coverage Report File ${filePath.split('/').pop()} does not exists!!`
    );
    return [];
  }
  replace(testResult);
...
const addSummaryHeadingAndTable = (core) => {
  ...
  const apiTestResult = getTestResult(
    'API Tests',
    API_TEST_RESULT,
    getCoverageReport(API_COVERAGE)
  );
  const e2eTestResult = getTestResult(
    'E2E Tests',
    E2E_TEST_RESULT,
    getCoverageReport(E2E_COVERAGE)
  );
  ...
  if (apiTestResult || e2eTestResult) {
    core.summary
      ...
      .addBreak()
      .addBreak()
      .addTable([tableHeader, apiTestResult, e2eTestResult]);
  }
};
